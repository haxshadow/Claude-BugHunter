...
# ---------------------------------------------------------------------------
# Normalize a response body for comparison: strip dynamic content (long hex
# tokens, timestamps, csrf/nonce/request-id values, digits, whitespace, case)
# so two semantically-identical pages compare equal. Used to decide whether a
# "bypass" body actually differs from the original 403/forbidden page or is
# just the same block page returned with a different status code.
# ---------------------------------------------------------------------------
_normalize_body() {
  local f="$1"
  [ -f "$f" ] || { printf ''; return; }
  LC_ALL=C sed -E \
    ...
    -e 's/(csrf|token|nonce|request[_-]?id|trace[_-]?id|session)[^a-zA-Z0-9]+[a-zA-Z0-9_-]+//Ig' \
    -e 's/[0-9]+//g' \
    "$f" 2>/dev/null | tr -d ' \t\r\n' | tr 'A-Z' 'a-z' | head -c 8000
}

# ---------------------------------------------------------------------------
# Confidence tier for a probe result, comparing the (normalized) bypass body
# against the (normalized) original response body:
#   [CONFIRMED]     status reached the backend (2xx) AND body differs from the
#                   original forbidden page  → a real, content-bearing bypass
#   [POSSIBLE]      status improved (redirect/401/5xx) and body differs        → worth manual review
#   [INFORMATIONAL] body identical to the original page, or no useful change   → likely cosmetic
# Args: <code> <bypass_norm>   (reads orig_norm from the calling scope)
# ---------------------------------------------------------------------------
_confidence_tier() {
  local code="$1" bypass_norm="$2"
  if [ "$bypass_norm" = "$orig_norm" ]; then
    printf '[INFORMATIONAL]'; return
  fi
  case "$code" in
    200|201|204)         printf '[CONFIRMED]' ;;
    301|302|401|500|502|503) printf '[POSSIBLE]' ;;
...
_probe_one() {
  ...
  # Baseline: the ORIGINAL (un-bypassed) response. Every probe body is compared
  # against this normalized baseline so a "200" that just re-serves the same
  # forbidden page is tiered [INFORMATIONAL], not a false-positive bypass.
  local _origbody _orighdr orig_code orig_norm
  _origbody=$(mktemp); _orighdr=$(mktemp)
  orig_code=$(curl -sk -D "$_orighdr" -o "$_origbody" -w "%{http_code}" --max-time 6 "$target" 2>/dev/null || echo "0")
  orig_norm=$(_normalize_body "$_origbody")
  local baseline_code=$orig_code
  log "baseline: original response HTTP $baseline_code ($(wc -c < "$_origbody" 2>/dev/null | tr -d ' ')B)"
  ...
  for combo in \
    ...
    case "$_verdict" in
      bypassed)
        ...
        local bypass_norm _tier
        bypass_norm=$(_normalize_body "$_tmpbody")
        _tier=$(_confidence_tier "$code" "$bypass_norm")
