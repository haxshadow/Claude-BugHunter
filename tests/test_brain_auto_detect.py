...
def test_openai_and_grok_keys_both_front_nothing_available(brain_module, monkeypatch):
    ...
    # the remaining providers keep their original PROVIDER_PRIORITY order
    # (minus the two key-bearing providers already probed up front)
    expected_tail = [p for p in brain_module.LLMClient.PROVIDER_PRIORITY
                     if p not in ("openai", "grok")]
    assert tracker.calls[2:] == expected_tail, \
